// ---------------------------------------------------------
// Titulo: Esqueletizacion de Imagenes y Poda Voraz
// Autores: Gabriel Ortiz Sepulveda
// Fecha de elaboracion: 24/03/2026
...
// --- Espacio de Nombres ---
// Usamos el espacio de nombres estandar para evitar escribir 'std::' repetidamente.
using namespace std;
...
void imprimirMatriz(const Matrix& img, string titulo) {
    cout << "\n" << titulo << ":" << endl; // Imprime el titulo proporcionado para la matriz.
    ...
    for (const auto& fila : img) {
        ...
        cout << endl; // Al final de cada fila, imprime un salto de linea para pasar a la siguiente.
    }
}
...
void crearHNormal(Matrix& img) {
    for (int i = 5; i <= 14; ++i) {
        img[i][5] = 1; img[i][6] = 1;   // Dibuja las dos lineas verticales de la 'H'.
        img[i][13] = 1; img[i][14] = 1; // Cada linea tiene 2 pixeles de grosor.
    }
    for (int j = 7; j <= 12; ++j) {
        img[9][j] = 1; img[10][j] = 1; // Dibuja la barra horizontal de la 'H', tambien de 2 pixeles de grosor.
    }
}
...
// Funcion para contar los vecinos activos (valor 1) de un pixel en (r, c) usando conectividad de 8.
int contarVecinos(const Matrix& m, int r, int c) {
    // Devuelve la suma de los valores de los 8 pixeles circundantes.
    // Dado que los pixeles son 0 (fondo) o 1 (forma), la suma es igual al numero de vecinos que son parte de la forma.
    return m[r-1][c] + m[r-1][c+1] + m[r][c+1] + m[r+1][c+1] +
           m[r+1][c] + m[r+1][c-1] + m[r][c-1] + m[r-1][c-1];
}

// Funcion para contar las transiciones de 0 a 1 en el vecindario de un pixel.
int contarTransiciones(const Matrix& m, int r, int c) {
    ...
    for (int i = 0; i < 8; ++i) {
        // Si un vecino es 0 (fondo) y el siguiente es 1 (forma), se ha encontrado una transicion.
        if (p[i] == 0 && p[i+1] == 1) count++;
    }
    return count; // Devuelve el numero total de transiciones 0->1.
}
...
void adelgazar(Matrix& img) {
    ...
    const int filas = img.size(); // Obtiene el numero de filas de la imagen. 'const' porque no cambiara.
    const int cols = img[0].size(); // Obtiene el número de columnas.

    // El bucle principal se ejecuta mientras 'cambio' sea verdadero, es decir, hasta que la imagen converge y no se borran mas pixeles.
    while (cambio) {
        ...
        vector<pair<int, int>> pixelesParaBorrar; // Vector para almacenar las coordenadas (fila, col) de los pixeles a borrar.

        // --- PASO 1 del algoritmo de Zhang-Suen ---
        // Se recorren todos los pixeles INTERIORES de la imagen (se omiten los bordes).
        for (int i = 1; i < filas - 1; ++i) {
            for (int j = 1; j < cols - 1; ++j) {
                if (img[i][j] == 1) { // Solo se consideran los pixeles que son parte de la forma.
                    int a = contarTransiciones(img, i, j); // A(p1): Numero de transiciones 0->1.
                    int b = contarVecinos(img, i, j);      // B(p1): Numero de vecinos con valor 1.
                    
                    // Se aplican las 4 condiciones del Paso 1 de Zhang-Suen:
                    if (a == 1 &&                           // 1. El numero de transiciones es 1.
                        (b >= 2 && b <= 6) &&                // 2. El numero de vecinos esta entre 2 y 6.
                        ...
                        pixelesParaBorrar.push_back({i, j}); // Si se cumplen todas, se marca el pixel para borrarlo.
                    }
                }
            }
        }
        // Despues de revisar toda la imagen, se procesan los pixeles marcados.
        if (!pixelesParaBorrar.empty()) { // Se comprueba si hay pixeles para borrar.
            // Se itera sobre la lista de pixeles a borrar.
        ...
        // Se recorren nuevamente todos los pixeles interiores.
        for (int i = 1; i < filas - 1; ++i) {
            for (int j = 1; j < cols - 1; ++j) {
                if (img[i][j] == 1) { // Solo se consideran los pixeles de la forma.
                    ...
                    if (a == 1 &&                           // 1. El numero de transiciones es 1.
                        ...
                        pixelesParaBorrar.push_back({i, j}); // Se marca el pixel para borrarlo.
                    }
                }
            }
        }
        // Se borran los pixeles marcados en el Paso 2.
        if (!pixelesParaBorrar.empty()) { // Se comprueba si hay pixeles para borrar.
            for (const auto& p : pixelesParaBorrar) { img[p.first][p.second] = 0; }
...
// Funcion para realizar una poda iterativa y segura del esqueleto.
// Elimina pixeles finales (ramas de ruido) hasta que la imagen deja de cambiar.
void podaVoraz(Matrix& img) {
    ...
    while (cambio) {
        cambio = false; // Se resetea la bandera al inicio de cada iteracion.
        vector<pair<int, int>> pixelesParaBorrar; // Buffer para almacenar los pixeles a podar.

        // Fase 1: Recolectar todos los puntos finales.
        // Se recorren los pixeles interiores.
        for (int i = 1; i < filas - 1; ++i) {
            for (int j = 1; j < cols - 1; ++j) {
                // Un pixel es un punto final si es parte de la forma (valor 1) y tiene 1 o 0 vecinos.
                if (img[i][j] == 1 && contarVecinos(img, i, j) <= 1) {
                    pixelesParaBorrar.push_back({i, j}); // Se anade a la lista de poda.
                }
            }
        }

        // Fase 2: Borrar los pixeles recolectados.
        if (!pixelesParaBorrar.empty()) { // Si se encontro al menos un pixel para podar...
            // Se itera sobre la lista y se elimina cada pixel (se pone a 0).
...
int main() {
    ...
    // Crea un vector de strings para almacenar los titulos de cada caso de prueba.
    vector<string> nombres = {"Caso 1: H Normal", "Caso 2: H con Ruido", "Caso 3: Cuadrado Relleno"};
    
    // Inicia un bucle 'for' que se ejecutara 3 veces, una para cada caso de prueba.
    for(int k = 0; k < 3; ++k) {
        // En cada iteracion, crea una nueva matriz 'img' del tamano especificado, inicializada con ceros.
        Matrix img(size, vector<int>(size, 0));
        
        // Usa una estructura 'switch' para llamar a la funcion de creacion de imagen correspondiente.
        switch(k) {
...
Temporal: O((K + P) * N * M), donde:
 ...
 - K: es el número de iteraciones del bucle 'while' en 'adelgazar'. K es proporcional al grosor máximo de la forma, ya que en cada pasada se elimina una "capa" de píxeles del borde. En cada iteración, se recorre la matriz completa (N*M), y las operaciones por píxel (`contarVecinos`, `contarTransiciones`) son de tiempo constante O(1).
 - P: es el numero de iteraciones en 'podaVoraz'. P es proporcional a la longitud de la rama mas larga a podar, porque en cada pasada se elimina solo el pixel final de la rama. En cada iteracion, tambien se recorre la matriz (N*M).
 - La complejidad total es la suma de los dos procesos principales que se ejecutan de forma secuencial.

Espacial: O(N * M), debido a:
 - La matriz 'img' que almacena la imagen, ocupando N * M espacio, es el componente dominante.
 - El vector 'pixelesParaBorrar'. En el peor de los casos (una forma de tablero de ajedrez), podria almacenar hasta la mitad de los pixeles, por lo que su espacio tambien es proporcional a N * M. No aumenta el orden de complejidad espacial.
*/